...
std::unique_ptr<Engine> Engine::build_mini_(const float* sample,
    ...
    mini->index_->dim = dim;
    mini->index_->path = "/dev/null";  // never written; just non-empty
    ...
    mini->index_->node_size =
        VamanaCore::static_node_size(mp.R, mini->index_->code_size);
    ...
    mini->index_->flat_store = std::make_unique<FlatNodeStore>(
        mini->index_->nodes_buffer, mini->index_->codes_buffer,
        mini->index_->node_size, mini->index_->code_size);
