...
struct TreeNodeHeader {
    uint32_t n_children;       // number of children in this node
    uint32_t extent_pages;     // total pages in this extent (for validation)
    uint16_t dim;              // centroid dimensionality
    uint16_t reserved;         // alignment
};
static_assert(sizeof(TreeNodeHeader) == 12);
...
struct ChildEntry {
    ...
    PageId   child_page;       // page index of the child's extent
    uint32_t child_pages;      // extent length in pages
...
static_assert(sizeof(ChildEntry) == 12);
...
inline uint64_t node_extent_bytes(uint16_t dim, uint32_t n_children) {
...
inline uint32_t node_extent_pages(uint16_t dim, uint32_t n_children) {
    return static_cast<uint32_t>(
        (node_extent_bytes(dim, n_children) + kPageSize - 1) / kPageSize);
}
...
struct TreeLeafHeader {
    uint32_t count;            // live vector count
    uint32_t tombstone_count;  // deleted vector count (Phase 4; 0 in Phase 1)
    ...
    uint32_t extent_pages;     // total pages in this extent
    // filter_summary follows after this struct (kFilterSummarySize bytes).
};
static_assert(sizeof(TreeLeafHeader) == 24);
...
inline uint32_t leaf_codes_offset() {
    return sizeof(TreeLeafHeader) + kFilterSummarySize;
}

/// Byte offset of the row_ids array within a leaf.
inline uint64_t leaf_rowids_offset(uint32_t n_blocks, uint32_t block_bytes) {
...
inline uint64_t leaf_factors_offset(uint32_t n_blocks, uint32_t block_bytes,
                                    uint32_t count) {
...
inline uint64_t leaf_extent_bytes(uint32_t count, uint16_t m4, uint8_t pq_bits,
                                  uint8_t n_factors) {
...
inline uint32_t leaf_extent_pages(uint32_t count, uint16_t m4, uint8_t pq_bits,
                                  uint8_t n_factors) {
    return static_cast<uint32_t>(
