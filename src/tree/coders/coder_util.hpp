...
/// Scan one scalar leaf into `sink` (bounded top-W max-heap). The zero
/// code row used by tail batches is a thread-local scratch (per-worker, so
/// parallel leaf scans never share mutable state).
inline void scalar_scan_leaf(const ScalarScanCtx& c, ScanSink& sink) {
    ...
    for (uint32_t i = 0; i < c.count; i += 4) {
        const uint32_t nv = std::min(4u, c.count - i);
        const uint8_t* cp[4];
        for (uint32_t v = 0; v < nv; ++v)
            cp[v] = c.codes + (uint64_t)(i + v) * cs;
        ...
        if (c.i8_mode) {
            ...
            for (uint32_t d = 0; d < d16; d += 16) {
                const float32x4_t a0 = vld1q_f32(&c.a_uni[d]);
                const float32x4_t a1 = vld1q_f32(&c.a_uni[d + 4]);
                const float32x4_t a2 = vld1q_f32(&c.a_uni[d + 8]);
                const float32x4_t a3 = vld1q_f32(&c.a_uni[d + 12]);
                for (uint32_t v = 0; v < 4; ++v) {
                    const uint8x8_t b = vld1_u8(cp[v] + d / 2);
                    const uint8x8_t lo = vand_u8(b, vdup_n_u8(0x0F));
                    const uint8x8_t hi = vshr_n_u8(b, 4);
            ...
            for (uint32_t d = 0; d < d4; d += 4) {
                const float a4[4] = {c.a_uni[d], c.a_uni[d + 1],
                                     c.a_uni[d + 2], c.a_uni[d + 3]};
        ...
        for (uint32_t v = 0; v < nv; ++v) {
            ...
            if (!sink.full()) {
                sink.push(pq_dist, i + v);
            } else if (pq_dist < sink.front()) {
                sink.replace(pq_dist, i + v);
            }
        }
    }
}
