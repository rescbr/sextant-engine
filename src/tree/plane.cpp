...
float PlaneIndex::scan_leaf_max_u8(uint32_t leaf_id,
    ...
    // (every pair's lane0 aliases the same u16 lanes — all even blocks
    // sum together), and 512-bit unpacklo/hi operate per-256-bit-half,
    // so the 128-bit FastScan lane model does not lift to 512-bit
    // pair-processing. Measured dead ends, see plane-round7 memory.
    const uint32_t nb = block_cnt_[leaf_id];
    const uint32_t count = leaf_cnt_[leaf_id];
...
void PlaneIndex::scan_leaf_max_u8_q(uint32_t leaf_id,
                                    const uint8_t* const* luts, uint32_t Q,
                                    const float* shifts, float* out) const {
    // Query tiling REVERTED to per-query calls (the tiled register
    // kernel was unsound — see scan_leaf_max_u8's note). API kept.
    for (uint32_t q = 0; q < Q; ++q)
        out[q] = scan_leaf_max_u8(leaf_id, luts[q],
                                  shifts ? shifts[q] : 0.0f);
}
